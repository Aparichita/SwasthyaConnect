        ...
        <div className="bg-white p-6 rounded-lg shadow h-[500px] overflow-y-auto">
          {messages.length === 0 ? (
            ...
            messages.map(msg => (
              <div
                key={msg._id}
                className={`flex ${msg.senderRole === 'patient' ? 'justify-end' : 'justify-start'} mb-3`}
              >
                <div className={`p-3 rounded-xl max-w-md ${msg.senderRole === 'patient' ? 'bg-primary-600 text-white' : 'bg-gray-100'}`}>
                  {msg.messageText}
                  {msg.attachmentUrl && (
